...
class TestResolveCommit:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                "b5b50f5d183ebc645460dba1329ee48798f31482-alpha",
                "b5b50f5d183ebc645460dba1329ee48798f31482-alpha",
            ],
        ],
    )
    ...
    def test_asks_to_fetch_latest_if_not_found(self, logged_in, project, commits, monkeypatch, confirm):
        ...
        with requests_mock.mock() as m:
            ...
            if confirm:
                ...
                monkeypatch.setattr(
                    "valohai_cli.utils.commits.fetch_latest_commits",
                    lambda unknown_commit: [{"identifier": new_commit}],
                )
    ...
    def test_fails_if_no_commits(self, logged_in, project, monkeypatch):
        with requests_mock.mock() as m:
            ...
            monkeypatch.setattr("valohai_cli.utils.commits.fetch_latest_commits", lambda unknown_commit: [])
            with pytest.raises(click.exceptions.Abort):
