...
from valohai_cli.commands.pipeline.stop import stop
...
def test_stop(runner, logged_in_and_linked, latest):
    counter = PIPELINE_DATA["counter"]

    with requests_mock.mock() as m:
        m.get(
            "https://app.valohai.com/api/v0/pipelines/",
            json={"results": [PIPELINE_DATA]},
        )
        m.get(
            f'https://app.valohai.com/api/v0/pipelines/{PIPELINE_DATA["project"]["id"]}:latest/',
            json=PIPELINE_DATA,
        )
        m.post(f'{PIPELINE_DATA["url"]}stop/', json={"message": "OK"})
