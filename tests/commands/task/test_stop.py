...
from valohai_cli.commands.task.stop import stop
...
def test_stop(runner, logged_in_and_linked, latest):
    counter = TASK_LIST_DATA["counter"]

    with requests_mock.mock() as m:
        m.get("https://app.valohai.com/api/v0/tasks/", json={"results": [TASK_LIST_DATA]})
        m.get(
            f'https://app.valohai.com/api/v0/tasks/{TASK_LIST_DATA["project"]}:latest/',
            json=TASK_LIST_DATA,
        )
        m.post(TASK_LIST_DATA["urls"]["stop"], json={"message": "OK"})
